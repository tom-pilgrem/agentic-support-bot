"""Stage 3: programmatic enforcement via PreToolUse/PostToolUse hooks.

Two rules the agent must never be able to talk its way around, no matter
...
Both are enforced here, not in the system prompt (see CLAUDE.md "Do not").
A prompt-only version of rule 1 is demonstrably bypassable — that's the
whole point of Stage 3's adversarial test. When either hook blocks a call,
